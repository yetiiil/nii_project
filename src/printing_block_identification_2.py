...
def main():
    ...
    model.load_state_dict(torch.load("/home/yuxiao/nii_project/siamese/content/model_contrastive.pth"))
    model.eval()
    ...
    df["block"] = np.nan
    file1, file2, charname, ssim, udistance, label = [], [], [], [], [], []

    for i in tqdm(range(len(jibo_list))):
        char = np.unique(df.loc[df["jibo"] == jibo_list[i], "char"])[0]
            ...
            #     df.loc[idx1, "block"] = count
                ...
                for idx2, row2 in df.loc[df["jibo"] == jibo_list[i]].iterrows():
                    ...
                    if idx2 > idx1:
                        ...
                        #     )
                        file1.append(row1.image_file)
                        file2.append(row2.image_file)
                        charname.append(char)
                        ssim.append(sim_score)
                        udistance.append(euclidean_distance.detach().cpu().numpy()[0])
                        label.append(jibo_list[i])

    df_plot = pd.DataFrame(list(zip(file1, file2, charname, label, ssim, udistance)), columns = ['image1', 'image2', "char", "label", "ssim", "euclidean_distance"])
    df_plot.to_csv("../dataset/df/df_plot.csv", index=False)
